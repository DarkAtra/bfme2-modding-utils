...
internal class EnumDeserializer<T : Enum<*>>(
    ...
    init {
        deserializationFunction = classOfT.companionObject!!.members
            ...
            .filter { function -> function.name.startsWith("of") }
            .firstOrNull(this::hasCorrectSignature)
            ?: error("Expected ${classOfT.simpleName} to have a companion object with exactly one function that starts with 'of'.")
    }

    override fun deserialize(inputStream: CountingInputStream): T {
        return deserializationFunction.call(
            classOfT.companionObjectInstance,
